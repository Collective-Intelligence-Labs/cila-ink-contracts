...
impl<'a> From<&'a str> for DomainEventType {
    fn from(s: &'a str) -> Self {
        match s {
            "UNSPECIFIED" => DomainEventType::UNSPECIFIED,
...
impl<'a> MessageWrite for DomainEvent<'a> {
    fn get_size(&self) -> usize {
        ...
        + if self.evnt_type == event::DomainEventType::UNSPECIFIED { 0 } else { 1 + sizeof_varint(*(&self.evnt_type) as u64) }
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if self.evnt_type != event::DomainEventType::UNSPECIFIED { w.write_with_tag(16, |w| w.write_enum(*&self.evnt_type as i32))?; }
