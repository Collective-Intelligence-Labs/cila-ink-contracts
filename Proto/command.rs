...
impl<'a> From<&'a str> for CommandType {
    fn from(s: &'a str) -> Self {
        match s {
            "UNSPECIFIED" => CommandType::UNSPECIFIED,
...
impl<'a> MessageWrite for Command<'a> {
    fn get_size(&self) -> usize {
        ...
        + if self.cmd_type == command::CommandType::UNSPECIFIED { 0 } else { 1 + sizeof_varint(*(&self.cmd_type) as u64) }
    ...
    fn write_message<W: WriterBackend>(&self, w: &mut Writer<W>) -> Result<()> {
        ...
        if self.cmd_type != command::CommandType::UNSPECIFIED { w.write_with_tag(24, |w| w.write_enum(*&self.cmd_type as i32))?; }
